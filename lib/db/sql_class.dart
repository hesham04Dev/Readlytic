...
class Sql {
  ...
  late BookFn books;
  late StatusFn statuses;
}

// NEW

// TODO remove the gift habit category
...
class BookFn {
  ...
  ResultSet get() {
    ResultSet resultSet = _db.select(''' SELECT * FROM books; ''');
    return resultSet;
  }

  add(Book book) {
    _db.execute(
        "insert into books(title,author,total_pages,current_page,average_reading_time,rating,categories,status_id,notes) values(?,?,?,?,?,?,?,?,?)",
        [
          book.title,
          book.author,
          book.totalPages,
          book.currentPage,
          book.averageReadingTime,
          book.rating,
          book.categories,
          book.statusId,
          book.notes
        ]);
    db.sql.settings.updateStreak();
  }

  update(Book book) {
    _db.execute(
      "UPDATE books SET title = ?, author = ?, total_pages = ?, current_page = ?, average_reading_time = ?, rating = ?, categories = ?, status_id = ?, notes = ? WHERE id = ?",
      [
        book.title,
        book.author,
        book.totalPages,
  ...
  addReading({required int bookId, required int readPages, required int time}) {
    _db.execute(
        "insert into reading_logs(book_id,reading_time,pages_read) values(?,?,?)",
        [bookId, time, readPages]);
    db.sql.settings.updateStreak();
  }

  ResultSet getWeeklyData() {
    var x = _db.select('''
      SELECT DATE(date) AS DateOnly, sum(reading_time) AS Total
                FROM reading_logs
                GROUP BY DateOnly
...
FROM books AS b
INNER JOIN reading_logs AS rl ON b.id = rl.book_id
GROUP BY DATE(date)
ORDER BY SUM(reading_time) DESC
LIMIT 1;
  ''');
...
class StatusFn {
  ...
  ResultSet get() {
    ResultSet resultSet = _db.select(''' SELECT * FROM status; ''');
    return resultSet;
  }

  
  Row getById(int id) {
    Row row = _db.select(''' SELECT * FROM status where id = ?; ''', [id])[0];
    return row;
  }
...
class SettingFn {
  ...
  String? _getLastDay() {
    ResultSet r =
        _db.select("select date from reading_logs Order by date DESC Limit 1");
    return r.isEmpty ? null : r[0]["date"];
  }
